...
export class FillUserTableDao {
  ...
  private readonly userImageUrlAttribute = "imageKey";
  private readonly passwordHashAttribute = "passwordHash";
  //   private readonly followeeCountAttribute = undefined;
  //   private readonly followerCountAttribute = undefined;
  ...
  private createPutUserRequest(user: User, hashedPassword: string) {
    const imageKey = this.extractImageKey(user.imageUrl);
    const item = {
      ...
      [this.userImageUrlAttribute]: imageKey,
  ...
  private extractImageKey(url: string): string {
    const key = url.substring(url.indexOf(".com/") + 5); // 5 = length of ".com/"
